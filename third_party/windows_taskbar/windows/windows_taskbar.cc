...
bool WindowsTaskbar::SetThumbnailToolbar(
  ...
  if (taskbar_) {
    ...
    if (image_list) {
      ...
      // buttons.
      // LOCAL PATCH (1/1): zero-initialize. The hidden filler entries below
      // only assign |dwMask|, |dwFlags| and |iId|, leaving |szTip| (a fixed
      // 260-WCHAR array inside the struct), |iBitmap| and |hIcon| holding
      // whatever was on the stack — and the whole array is then marshalled
      // across the process boundary into explorer.exe. Microsoft's own sample
      // writes `THUMBBUTTON buttons[3] = {};`. See PATCHES.md.
