...
class _TemplateDetailPageState extends ConsumerState<TemplateDetailPage> {
  ...
  Widget build(BuildContext context) {
    ...
    final mainFile = File(
      p.join(currentTemplate.sourceFolderPath, currentTemplate.mainFileRelativePath),
    );
    ...
    final projectsFromTemplate = (ref.watch(allProjectsStreamProvider).value ?? const [])
        .where((p) => p.sourceTemplateId == currentTemplate.id)
        .toList();

    return Scaffold(
      appBar: AppBar(title: Text(currentTemplate.name)),
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
              ...
              children: [
                OutlinedButton.icon(
                  onPressed: sourceExists
                      ? () => FileLauncher.openFolder(currentTemplate.sourceFolderPath)
                      : null,
                  icon: const Icon(Icons.folder_open, size: 16),
                  label: Text(l10n.openFolder),
                ),
                OutlinedButton.icon(
                  onPressed: sourceExists ? () => _useTemplate(currentTemplate) : null,
                  icon: const Icon(Icons.add_circle_outline, size: 16),
                  label: Text(l10n.useTemplate),
                ),
                OutlinedButton.icon(
                  onPressed: sourceExists ? () => _duplicateTemplate(currentTemplate) : null,
              ...
              children: [
                ...
                if (dawType != null) ...[
                  ...
                  Expanded(
                    child: TextFormField(
                      enabled: false,
                      initialValue: currentTemplate.dawVersion?.isNotEmpty == true
                          ? '$dawType ${currentTemplate.dawVersion}'
                          : dawType,
                      decoration: InputDecoration(
                        labelText: l10n.daw,
                        border: const OutlineInputBorder(),
                        prefixIcon: _buildDawPrefixIcon(dawType, Theme.of(context).colorScheme.primary),
                      ),
                    ),
                  ),
                ],
            ...
            Text(l10n.fileInfo, style: Theme.of(context).textTheme.titleMedium),
              ...
              Card(
                child: Padding(
                  ...
                  child: Column(
                    ...
                    children: [
                      Text('${l10n.fileSize}: ${_formatFileSize(mainFile.lengthSync())}'),
                      const SizedBox(height: 4),
                      SelectableText('${l10n.filePath}: ${mainFile.path}'),
                      const SizedBox(height: 4),
                      Text(
                  ...
                  for (final project in projectsFromTemplate)
                    Card(
                      ...
                      child: ListTile(
                        ...
                        subtitle: Text(dateFormat.format(project.createdAt)),
